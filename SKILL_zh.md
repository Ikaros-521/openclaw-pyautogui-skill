...
# 一条命令完成复制并粘贴（长文本输入最快）
python3 scripts/keyboard_mouse.py copy_paste "要直接输入的文本"
```

**使用建议：**
- `copy_paste` 适合长文本，通常比 `type_text` 更快
- 需要模拟逐字输入效果时，使用 `type_text`
...
| `screenshot_region path x1 y1 x2 y2` | 区域截图 | `keyboard_mouse.py screenshot_region region.png 100 100 500 500` |
| `copy text` | 复制到剪贴板 | `keyboard_mouse.py copy "Hello"` |
| `paste` | 从剪贴板粘贴 | `keyboard_mouse.py paste` |
| `copy_paste text` | 复制并粘贴 | `keyboard_mouse.py copy_paste "Hello World"` |
